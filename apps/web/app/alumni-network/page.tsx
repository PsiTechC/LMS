...
const BENEFITS: { icon: string; title: string; body: string; color: string }[] = [
  { icon: "◈", title: "Lifetime Network Access", body: "Stay connected to your cohort and the wider alumni community long after your program ends.", color: "#EF4E24" },
  { icon: "✦", title: "Exclusive Events", body: "Invitations to alumni-only masterclasses, panels, and regional meetups throughout the year.", color: "#1C2551" },
  { icon: "◎", title: "Peer Mentorship", body: "Give or get mentorship from leaders across industries who share your program background.", color: "#6B73BF" },
...
const STORIES: { name: string; role: string; program: string; quote: string; color: string }[] = [
  { name: "Aisha Khan", role: "VP Operations, Northwind Retail", program: "Leadership Accelerator – Batch 4", quote: "The alumni network is where the real ROI shows up — I still call two people from my cohort for advice every month.", color: "#EF4E24" },
  { name: "Rahul Menon", role: "Director of Engineering, Fintra", program: "Senior Manager Fast Track", quote: "Two years out, and the peer group from this program is still my sharpest sounding board for tough calls.", color: "#1C2551" },
  { name: "Priya Nataraj", role: "Head of People, Solvix", program: "Women in Leadership", quote: "The alumni events aren't networking for the sake of it — they're genuinely where I've found my next mentors.", color: "#6B73BF" },
];
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#F5F7FB", fontFamily: "Poppins,sans-serif" }}>
      ...
      <div style={{ background: "linear-gradient(135deg,#0f1635 0%,#1C2551 55%,#0f1635 100%)", padding: "60px 24px 52px", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: 900, margin: "0 auto", position: "relative" }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 7, background: "rgba(239,78,36,0.15)", border: "1px solid rgba(239,78,36,0.35)", borderRadius: 20, padding: "4px 14px", marginBottom: 20 }}>
            ...
            <span style={{ fontSize: 11, color: "#EF4E24", fontWeight: 700, letterSpacing: 0.5 }}>ALUMNI NETWORK</span>
          </div>
          <div style={{ fontSize: "clamp(28px, 6vw, 42px)", fontWeight: 800, color: "#fff", marginBottom: 14, lineHeight: 1.15, maxWidth: 640 }}>
            The network doesn&apos;t end<br /><span style={{ color: "#EF4E24" }}>when the program does.</span>
          </div>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "56px 24px 8px" }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: "#EF4E24", letterSpacing: 1, marginBottom: 8, textAlign: "center" }}>MEMBER BENEFITS</div>
        <div style={{ fontSize: 22, fontWeight: 700, color: "#1C2551", marginBottom: 34, textAlign: "center" }}>What alumni get, for life</div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 18 }}>
          {BENEFITS.map(b => (
            <div key={b.title} style={{ background: "#fff", borderRadius: 14, border: "1px solid #EAECF4", padding: "22px 20px", boxShadow: "0 1px 6px rgba(28,37,81,0.06)" }}>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "56px 24px" }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: "#EF4E24", letterSpacing: 1, marginBottom: 8, textAlign: "center" }}>ALUMNI STORIES</div>
        <div style={{ fontSize: 22, fontWeight: 700, color: "#1C2551", marginBottom: 34, textAlign: "center" }}>Where our alumni are now</div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 20 }}>
          {STORIES.map(s => (
            <div key={s.name} style={{ background: "#fff", borderRadius: 16, border: "1px solid #EAECF4", padding: "24px 22px", boxShadow: "0 1px 6px rgba(28,37,81,0.06)", display: "flex", flexDirection: "column" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <div>
                  <div style={{ fontSize: 12.5, fontWeight: 700, color: "#1C2551" }}>{s.name}</div>
                  <div style={{ fontSize: 11, color: "#8b90a7" }}>{s.role}</div>
      ...
      <div style={{ background: "#1C2551", padding: "48px 24px", textAlign: "center" }}>
        ...
        <a href="/open-programs" style={{ display: "inline-block", padding: "11px 24px", background: "#EF4E24", borderRadius: 22, color: "#fff", fontSize: 13, fontWeight: 700, textDecoration: "none" }}>Browse Programs →</a>
      </div>
