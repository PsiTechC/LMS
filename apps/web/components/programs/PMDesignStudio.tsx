...
import { programsApi, ProgramDetailDTO, ActivityFacultyDTO, OrgFacultyMember, ConflictDTO, ScheduledSessionDTO } from "@/lib/programs-api";
      ...
      <div style={{display:"flex",flex:1,overflow:"hidden"}}>
                    ...
                    return (
                      <div key={act.id}
                        ...
                        <div onMouseDown={e=>beginDrag(e,"a-move",ph.id,act.id)} style={{flex:1,padding:"4px 7px 3px",cursor:"grab",display:"flex",flexDirection:"column",gap:1,overflow:"hidden"}}>
                          ...
                          {/* Quick schedule indicator */}
                          {isFacultyTarget&&aH>48&&(
                            <div style={{fontSize:7,color:"rgba(255,255,255,0.7)",paddingLeft:13,marginTop:1,fontWeight:600}}>📅 Click → Schedule</div>
                          )}
                        </div>
        ...
        {sel&&selPh?(
          <RPanel ph={selPh} act={selAct} total={totalDays} sd={startDate}
            programId={program.id} orgId={orgId||""}
...
// ─── Right config panel ───────────────────────────────────────────────────────
function RPanel({ph,act,total,sd,programId,orgId,orgFaculty,onClose,onUpdPh,onUpdAct,onAddAct,onAssignFaculty,onRemoveFaculty}:{
  ph:Ph;act:Act|null;total:number;sd:Date|null;
  programId:string;orgId:string;
  ...
  const [schedForm,setSchedForm]=useState({cohort_id:"",faculty_id:"",title:"",scheduled_at:"",duration_mins:60,session_type:"classroom",virtual_link:""});
  const [schedSaving,setSchedSaving]=useState(false);
  const [schedSessions,setSchedSessions]=useState<ScheduledSessionDTO[]>([]);
  useEffect(()=>setPhName(ph.label),[ph.id]);
  useEffect(()=>{setATitle(act?.title||"");setFacPick(false);setShowSchedModal(false);},[act?.id]);

  const isSessionAct = act && (act.type==="live_session"||act.type==="coaching");

  useEffect(()=>{
    if(!isSessionAct||!act.id||!programId) return;
    const uuidRe=/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
    if(!uuidRe.test(act.id)||!uuidRe.test(programId)) return;
    programsApi.listActivitySessions(programId,act.id)
      .then(r=>setSchedSessions(r.data??[]))
      .catch(()=>{});
  },[act?.id,programId,isSessionAct]);

  async function openSchedModal() {
    if(orgId&&programId){
      try{const r=await cohortsApi.list(orgId,programId);setCohorts(r.data??[]);}catch(_e){}
    }
    const defaultTitle=`${act?.title||"Session"} – ${fmt(new Date())}`;
    setSchedForm({cohort_id:"",faculty_id:(act?.faculty??[])[0]?.faculty_user_id??"",title:defaultTitle,scheduled_at:"",duration_mins:act?.durationMins??60,session_type:"classroom",virtual_link:""});
    setShowSchedModal(true);
  }

  async function submitSchedForm() {
    if(!schedForm.cohort_id||!schedForm.faculty_id||!schedForm.scheduled_at||!act||!programId) return;
    setSchedSaving(true);
    try{
      const r=await programsApi.scheduleSession(programId,act.id,{
        program_id:programId, cohort_id:schedForm.cohort_id, faculty_id:schedForm.faculty_id,
        title:schedForm.title, session_type:schedForm.session_type, virtual_link:schedForm.virtual_link,
        scheduled_at:new Date(schedForm.scheduled_at).toISOString(), duration_mins:schedForm.duration_mins,
      });
      if(r.data) setSchedSessions(prev=>[...prev,r.data!]);
      setShowSchedModal(false);
    }catch(_e){}finally{setSchedSaving(false);}
  }
    ...
    <div style={{width:240,flexShrink:0,background:C.card,borderLeft:`1px solid ${C.border}`,display:"flex",flexDirection:"column",overflow:"hidden"}}>
      ...
      <div style={{flex:1,overflow:"auto",padding:"11px 12px",display:"flex",flexDirection:"column",gap:11}}>
        {act?(<>
          ...
          {/* Session scheduling — live_session, coaching, classroom_event */}
          {isSessionAct&&(
            <div style={{borderTop:`1px solid ${C.border}`,paddingTop:8}}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:5}}>
                <div style={{fontSize:8,fontWeight:700,color:C.muted,letterSpacing:0.5}}>SCHEDULED SESSIONS</div>
                <button onClick={openSchedModal} style={{border:"none",background:C.navy+"14",borderRadius:5,cursor:"pointer",fontFamily:"Poppins,sans-serif",color:C.navy,fontWeight:700,fontSize:9,padding:"3px 7px"}}>+ Schedule</button>
              </div>
              {schedSessions.length===0&&<div style={{fontSize:9,color:C.muted}}>No sessions scheduled yet.</div>}
              {schedSessions.map(s=>(
                <div key={s.id} style={{padding:"4px 0",borderBottom:`1px solid ${C.border}`,display:"flex",flexDirection:"column",gap:1}}>
                  <div style={{fontSize:10,fontWeight:600,color:C.navy}}>{s.title}</div>
                  <div style={{fontSize:8,color:C.muted}}>{s.scheduled_at?new Date(s.scheduled_at).toLocaleDateString():""} · {s.duration_mins}min{s.faculty_name?` · ${s.faculty_name}`:""}</div>
                </div>
              ))}
              {/* Schedule Session Modal */}
              {showSchedModal&&(
                <div style={{position:"fixed",inset:0,background:"rgba(28,37,81,0.5)",zIndex:3000,display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
                  <div style={{background:C.card,borderRadius:16,width:440,maxWidth:"100%",boxShadow:"0 24px 64px rgba(28,37,81,0.22)",overflow:"hidden"}}>
                    <div style={{padding:"16px 20px",borderBottom:`1px solid ${C.border}`,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                      <div style={{fontSize:14,fontWeight:700,color:C.navy}}>Schedule Session</div>
                      <button onClick={()=>setShowSchedModal(false)} style={{border:"none",background:"transparent",color:C.muted,cursor:"pointer",fontSize:14}}>✕</button>
                    </div>
                    <div style={{padding:"16px 20px",display:"flex",flexDirection:"column",gap:12}}>
                      <FG label="TITLE"><input value={schedForm.title} onChange={e=>setSchedForm(f=>({...f,title:e.target.value}))} style={inSt}/></FG>
                      <FG label="COHORT">
                        <select value={schedForm.cohort_id} onChange={e=>setSchedForm(f=>({...f,cohort_id:e.target.value}))} style={inSt}>
                      ...
                      <FG label="FACULTY">
                        <select value={schedForm.faculty_id} onChange={e=>setSchedForm(f=>({...f,faculty_id:e.target.value}))} style={inSt}>
                          <option value="">— Select faculty —</option>
                          {(act?.faculty??[]).map(f=><option key={f.faculty_user_id} value={f.faculty_user_id}>{f.name} ({f.role})</option>)}
                      ...
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                        <FG label="DATE &amp; TIME"><input type="datetime-local" value={schedForm.scheduled_at} onChange={e=>setSchedForm(f=>({...f,scheduled_at:e.target.value}))} style={inSt}/></FG>
                        <FG label="DURATION (MINS)"><input type="number" min={5} max={480} step={5} value={schedForm.duration_mins} onChange={e=>setSchedForm(f=>({...f,duration_mins:+e.target.value}))} style={inSt}/></FG>
                      </div>
                      <FG label="SESSION TYPE">
                        <select value={schedForm.session_type} onChange={e=>setSchedForm(f=>({...f,session_type:e.target.value}))} style={inSt}>
                      ...
                      <FG label="VIRTUAL LINK (optional)"><input value={schedForm.virtual_link} onChange={e=>setSchedForm(f=>({...f,virtual_link:e.target.value}))} placeholder="https://..." style={inSt}/></FG>
                    </div>
                    <div style={{padding:"12px 20px",borderTop:`1px solid ${C.border}`,display:"flex",gap:8,justifyContent:"flex-end"}}>
                      <button onClick={()=>setShowSchedModal(false)} style={{padding:"8px 16px",border:`1px solid ${C.border}`,borderRadius:8,cursor:"pointer",fontFamily:"Poppins,sans-serif",color:C.muted,fontWeight:600,fontSize:12,background:C.card}}>Cancel</button>
                      <button onClick={submitSchedForm} disabled={schedSaving||!schedForm.cohort_id||!schedForm.faculty_id||!schedForm.scheduled_at} style={{padding:"8px 20px",border:"none",borderRadius:8,cursor:schedSaving?"not-allowed":"pointer",fontFamily:"Poppins,sans-serif",color:"#fff",fontWeight:700,fontSize:12,background:C.navy,opacity:(schedSaving||!schedForm.cohort_id||!schedForm.faculty_id||!schedForm.scheduled_at)?0.5:1}}>
                        {schedSaving?"Saving…":"Create Session"}
                      </button>
                    </div>
                  </div>
                </div>
