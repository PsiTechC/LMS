...
func ensureZoomMeeting(s *ClassSession, callerID, callerRole string) (string, error) {
	...
	payload := map[string]any{
		"topic":            s.Title,
		"start_time":       s.ScheduledAt.UTC().Format(time.RFC3339),
	...
	req, err := http.NewRequest(http.MethodPost, internalAPIBaseURL()+"/sessions/"+s.ID.String()+"/zoom-meeting", bytes.NewReader(body))
	if err != nil {
		return "", err
	}
	...
	resp, err := zoomBridgeClient.Do(req)
	if err != nil {
